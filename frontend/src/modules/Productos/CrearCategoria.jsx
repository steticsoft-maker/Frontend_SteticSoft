...
const CrearCategoria = () => {
  ...
  const confirmCreateCategoria = () => {
    const vidaUtil = parseInt(vidaUtilCategoria, 10);


    const storedCategorias = localStorage.getItem("categorias");
    const categorias = storedCategorias ? JSON.parse(storedCategorias) : [];

    const newId = categorias.length > 0 ? Math.max(...categorias.map(cat => cat.id)) + 1 : 1;
    ...
    localStorage.setItem("categorias", JSON.stringify([...categorias, newCategoria]));
      ...
      {showConfirmModal && (
        <div className="modal-compras"> 
          <div className="modal-content-compras">
            ...
            <p>¿Está seguro de que desea crear esta nueva categoría?</p>
            <div className="modal-compras-buttons-anular">
              <button
                className="botonConfirmarAnularCompra"
                onClick={confirmCreateCategoria}
              >
                Sí, crear
              </button>
              <button
