...
const useAbastecimiento = () => {
  ...
  const handleOpenModal = useCallback((type, entry = null) => {
    ...
    if (type === "details") setIsDetailsModalOpen(true);
        ...
        if (entry?.estaAgotado) {
            setValidationMessage("No se puede editar un registro de producto agotado.");
            setIsValidationModalOpen(true);
            return;
        }
  ...
  const handleSubmitForm = useCallback(async (formData, isCreating = false) => {
    ...
    try {
      ...
      setValidationMessage(err.response?.data?.message || err.message || "Error al guardar el registro.");
      setIsValidationModalOpen(true);
    } finally {
