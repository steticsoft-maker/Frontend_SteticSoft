...
const useUsuarios = () => {
  ...
  const validateField = useCallback(
    (name, value, currentFormData, formType = "create") => {
      let error = "";
      ...
      const selectedRoleObj = availableRoles.find(
        (r) => r.idRol === parseInt(currentFormData.idRol)
      );
      ...
      switch (name) {
        ...
        case "confirmarContrasena":
          if (formType === "create" && !value)
            ...
            formType === "create" &&
            value !== currentFormData.contrasena
          )
  ...
  const runValidations = useCallback(
    (dataToValidate, formType) => {
      ...
      const selectedRoleObj = availableRoles.find(
        (r) => r.idRol === parseInt(dataToValidate.idRol)
      );
      // Usar tipoPerfil para determinar si se requieren campos de perfil
      const requiresProfile =
        selectedRoleObj &&
    ...
    [validateField, availableRoles, formErrors.correo]
  ); // Depender de formErrors.correo

  // Efecto para actualizar isFormValid cuando formErrors o formData cambian
  useEffect(() => {
    if (Object.keys(formData).length > 0 && !isVerifyingEmail) {
      const { errors, isValid: currentFieldsValid } = runValidations(
        formData,
        formData.idUsuario ? "edit" : "create"
      );
  ...
  const cargarDatos = useCallback(async () => {
    ...
    try {
      ...
      setErrorPage(
        err.message || "No se pudieron cargar los datos de usuarios o roles."
      );
      setUsuarios([]);
      setAvailableRoles([]);
    } finally {
      ...
      if (
        ...
        setValidationMessage(
          `El usuario 'Administrador' no puede ser ${
            type === "edit" ? "editado" : "eliminado/desactivado"
          }.`
        );
      ...
      if (name === "correo" && !error && value) {
        ...
        if (value !== originalEmail) {
          ...
          setFormErrors((prevErrors) => ({
            ...prevErrors,
            correo: error || "",
          })); // También considerar el error de formato síncrono
        }
      }
    },
  ...
  const handleSaveUsuario = useCallback(async () => {
    ...
    try {
      ...
      const selectedRoleObj = availableRoles.find(
        (r) => r.idRol === parseInt(dataParaAPI.idRol)
      );
      ...
      if (!shouldHaveProfile) {
        ...
        [
          ...
          "telefono",
          "fechaNacimiento",
        ].forEach((field) => delete dataParaAPI[field]);
      }
