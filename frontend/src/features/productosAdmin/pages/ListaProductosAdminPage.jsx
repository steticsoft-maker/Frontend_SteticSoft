...
function ListaProductosAdminPage() {
  ...
  const cargarProductos = useCallback(async () => {
    // ✨ Eliminamos searchTerm de los parámetros porque el filtrado ahora es local
    setIsLoading(true);
    setError(null);
    try {
  ...
  const handleSave = async (productoData) => {
    try {
      if (productoData.idProducto) {
        await productosAdminService.updateProducto(
          productoData.idProducto,
          productoData
        );
        MySwal.fire("¡Éxito!", "Producto actualizado exitosamente.", "success");
      } else {
        const response = await productosAdminService.createProducto(
          productoData
        );
        if (response?.errors) {
