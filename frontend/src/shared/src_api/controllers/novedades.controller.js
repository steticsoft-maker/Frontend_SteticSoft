...
const crearNovedad = async (req, res, next) => {
  try {
    ...
    const nuevaNovedad = await novedadesService.crearNovedad(
      datosNovedad,
      empleadosIds
    );
    res.status(201).json({
...
const listarNovedades = async (req, res, next) => {
  try {
    ...
    const novedades =
      await novedadesService.obtenerTodasLasNovedades(opcionesDeFiltro);
    res.status(200).json({
      success: true,
      data: novedades,
    });
...
const listarDiasDisponibles = async (req, res, next) => {
  try {
    ...
    const dias = await novedadesService.obtenerDiasDisponibles(
      Number(idNovedad),
      anio,
      mes
    );
...
const listarHorasDisponibles = async (req, res, next) => {
  try {
    ...
    const horas = await novedadesService.obtenerHorasDisponibles(
      Number(idNovedad),
      fecha
    );
...
const listarEmpleadosPorNovedad = async (req, res, next) => {
  try {
    const { idNovedad } = req.params;
    const empleados = await novedadesService.obtenerEmpleadosPorNovedad(
      Number(idNovedad)
    );
