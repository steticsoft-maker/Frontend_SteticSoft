  ...
  body("idCategoriaProducto")
    ...
    .custom(async (value) => {
      const categoria = await db.CategoriaProducto.findOne({
        where: { idCategoriaProducto: value, estado: true },
      });
      if (!categoria) {
        throw new Error(`Categoría con ID ${value} no encontrada o inactiva.`);
      }
      return true;
    }),
  ...
  body("nombre")
    .custom(async (value) => {
      const productoExistente = await db.Producto.findOne({
        where: { nombre: value.trim() },
      });
      if (productoExistente) {
        throw new Error(`Ya existe un producto con el nombre "${value}".`);
      }
      return true;
    }),
