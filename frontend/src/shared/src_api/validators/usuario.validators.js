...
const db = require("../models/index.js"); // Ensure db is available
...
const crearUsuarioValidators = [
    ...
    .custom(async (value) => {
      ...
      if (usuarioExistente) {
        return Promise.reject(
          "The email address is already registered for a user account."
        );
      }
    }),
  body("contrasena")
  ...
  body("idRol")
    ...
    .custom(async (value) => {
      ...
      if (!rolExistente) {
        return Promise.reject(
          "The specified role does not exist or is not active."
        );
      }
    }),
  body("estado")
    .optional()
    .isBoolean()
    .withMessage("The status must be a boolean value (true or false)."),
  ...
  body("apellido")
    ...
    .isString()
    .withMessage("The last name must be a text string.")
    .isLength({ min: 2, max: 100 })
  ...
  body("telefono")
    ...
    .isString()
    .withMessage("The phone number must be a text string.") // You could add isNumeric or a specific regex if you want.
  ...
  body("numeroDocumento")
    ...
    .custom(async (value, { req }) => {
      ...
      if (rol.nombre === "Cliente") {
        ...
        if (clienteExistente) {
          return Promise.reject(
            "The document number is already registered for a client."
          );
        }
        ...
        if (empleadoExistente) {
          return Promise.reject(
            "The document number is already registered for an employee."
          );
        }
      }
      return true;
...
const actualizarUsuarioValidators = [
  ...
  body("contrasena") // Password is optional on update
    ...
    .isLength({ min: 8 })
    .withMessage(
      "The password must be at least 8 characters long if updating."
    ),
  body("idRol")
    .optional()
    .isInt({ gt: 0 })
    .withMessage("The role ID must be a positive integer if updating.")
    .custom(async (value) => {
      if (value) {
        // Only validate if an idRol is provided
        const rolExistente = await db.Rol.findOne({
          where: { idRol: value, estado: true },
        });
        if (!rolExistente) {
          return Promise.reject(
            "The specified role for update does not exist or is not active."
          );
        }
      }
    }),
  ...
  body("nombre")
    ...
    .trim()
    .notEmpty()
    .withMessage("The name cannot be empty if updating.")
    .isLength({ min: 2, max: 100 }),
  body("apellido")
    .optional()
    .trim()
    .notEmpty()
    .withMessage("The last name cannot be empty if updating.")
    .isLength({ min: 2, max: 100 }),
  body("telefono")
    .optional()
    .trim()
    .notEmpty()
    .withMessage("The phone number cannot be empty if updating.")
    .isLength({ min: 7, max: 45 }),
  body("tipoDocumento")
    .optional()
    .trim()
    .notEmpty()
    .withMessage("The document type cannot be empty if updating.")
    .isIn([
      "Cédula de Ciudadanía",
      "Cédula de Extranjería",
      "Pasaporte",
  ...
  body("numeroDocumento")
    ...
    .trim()
    .notEmpty()
    .withMessage("The document number cannot be empty if updating.")
    .isLength({ min: 5, max: 45 })
    .custom(async (value, { req }) => {
      if (!value) return true;
      const idUsuario = Number(req.params.idUsuario); // Get the idUsuario being updated
      const usuarioActual = await db.Usuario.findByPk(idUsuario, {
        include: ["rol"],
      });
      ...
      if (rolNombre === "Cliente") {
        const clienteAsociado = await db.Cliente.findOne({
          where: { idUsuario },
        });
        if (clienteAsociado && clienteAsociado.numeroDocumento === value)
          return true; // It's the same document of the current client, no conflict
        const otroClienteConDocumento = await db.Cliente.findOne({
          where: { numeroDocumento: value },
        });
        if (otroClienteConDocumento) {
          return Promise.reject(
            "The document number is already registered for another client."
          );
        }
      } else if (rolNombre === "Empleado") {
        const empleadoAsociado = await db.Empleado.findOne({
          where: { idUsuario },
        });
        if (empleadoAsociado && empleadoAsociado.numeroDocumento === value)
          return true;
