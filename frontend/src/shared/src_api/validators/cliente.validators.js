...
const clienteCreateValidators = [
  ...
  body("fechaNacimiento")
    ...
    .isISO8601()
    .withMessage("Formato de fecha no válido (debe ser YYYY-MM-DD).")
    .toDate()
    .custom((value) => {
...
const clienteUpdateValidators = [
  param("idCliente")
    .isInt({ gt: 0 })
    .withMessage("El ID del cliente es inválido."),
  ...
  body("numeroDocumento")
    ...
    .custom(async (value, { req }) => {
      ...
      if (cliente) {
        return Promise.reject(
          "Este número de documento ya está en uso por otro cliente."
        );
      }
    }),
...
const updateMiPerfilValidators = [
  ...
  body("tipoDocumento")
    .not()
    .exists()
    .withMessage("El tipo de documento no se puede modificar."),
  body("numeroDocumento")
    .not()
    .exists()
    .withMessage("El número de documento no se puede modificar."),
