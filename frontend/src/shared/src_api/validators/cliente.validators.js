...
const crearClienteValidators = [
  ...
  body("tipoDocumento")
    ...
    ]) // Asegúrate que estos valores sean los que usas
  ...
  body("estadoCliente") // Estado para el perfil del Cliente
    .optional()
    .isBoolean()
    .withMessage("El estado del cliente debe ser un valor booleano."),

  // --- Campos para la Cuenta de Usuario asociada ---
  body("correo")
    .trim()
    .notEmpty()
    .withMessage("El correo electrónico para la cuenta es obligatorio.")
    .isEmail()
    ...
    .custom(async (value) => {
      ...
      if (usuarioExistente) {
        return Promise.reject(
          "El correo electrónico ya está registrado para una cuenta de usuario."
        );
      }
      ...
      if (clienteExistente) {
        return Promise.reject(
          "El correo electrónico ya está registrado para un perfil de cliente."
        );
      }
    }),
  body("contrasena")
    .notEmpty()
    .withMessage("La contraseña para la cuenta es obligatoria.")
    .isString()
  ...
  body("estadoUsuario") // Estado para la cuenta Usuario
    .optional()
    .isBoolean()
    .withMessage("El estado del usuario debe ser un valor booleano."),
...
const actualizarClienteValidators = [
  ...
  body("idUsuario")
    ...
    .custom(async (value, { req }) => {
      if (value) {
        // Solo validar si no es null y es un número
        const idCliente = Number(req.params.idCliente);
