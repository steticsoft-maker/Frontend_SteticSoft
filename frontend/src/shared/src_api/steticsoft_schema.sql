...
CREATE TABLE IF NOT EXISTS rol (
    ...
    tipo_perfil VARCHAR(10) NOT NULL DEFAULT 'EMPLEADO' CHECK (
        tipo_perfil IN ('CLIENTE', 'EMPLEADO', 'NINGUNO')
    ),
    descripcion TEXT,
...
CREATE TABLE IF NOT EXISTS permisos_x_rol (
    ...
    asignado_por INT REFERENCES usuario(id_usuario) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS historial_cambios_rol (
    ...
    id_usuario_modifico INT REFERENCES usuario(id_usuario) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS compra (
    ...
    id_dashboard INT REFERENCES dashboard(id_dashboard) ON DELETE
    SET NULL,
        estado BOOLEAN DEFAULT TRUE NOT NULL
);
...
CREATE TABLE IF NOT EXISTS novedad_empleado (
    ...
    CONSTRAINT fk_novedad FOREIGN KEY (id_novedad) REFERENCES novedades(id_novedad) ON DELETE CASCADE,
    -- Si se borra la novedad, se borra la asignación
    -- Llave foránea que apunta a la tabla de usuarios (empleados)
    CONSTRAINT fk_usuario FOREIGN KEY (id_usuario) REFERENCES usuario(id_usuario) ON DELETE CASCADE -- Si se borra el usuario, se borra la asignación
);
...
CREATE TABLE IF NOT EXISTS venta_x_servicio (
    ...
    id_cita INT REFERENCES cita(id_cita) ON DELETE
    SET NULL,
        id_venta INT REFERENCES venta(id_venta) ON DELETE CASCADE
);
CREATE TABLE IF NOT EXISTS abastecimiento (
    ...
    estado BOOLEAN DEFAULT TRUE NOT NULL,
    id_usuario INT REFERENCES usuario(id_usuario) ON DELETE
    SET NULL
);
...
-- =================================================================================================
INSERT INTO rol (nombre, tipo_perfil, descripcion, estado)
VALUES (
        'Administrador',
        'NINGUNO',
...
UPDATE
SET tipo_perfil = EXCLUDED.tipo_perfil,
    descripcion = EXCLUDED.descripcion,
    estado = EXCLUDED.estado;
INSERT INTO permisos (nombre, descripcion, estado)
VALUES (
        'MODULO_ROLES_GESTIONAR',
        'Permite la gestión completa de roles del sistema.',
        TRUE
    ),
    (
        'MODULO_ROLES_ASIGNAR_PERMISOS',
        'Permite asignar y quitar permisos a los roles.',
        TRUE
    ),
    (
        'MODULO_PERMISOS_GESTIONAR',
        'Permite la gestión completa de los permisos del sistema.',
        TRUE
    ),
    (
        'MODULO_USUARIOS_GESTIONAR',
        'Permite la gestión completa de usuarios del sistema.',
        TRUE
    ),
    (
        'MODULO_DASHBOARD_VER',
        'Permite visualizar los dashboards y sus datos.',
        TRUE
    ),
    (
        'MODULO_ESTADOS_GESTIONAR',
    ...
    (
        'MODULO_CLIENTES_VER_PROPIO',
        'Permite a un cliente ver y editar su propio perfil.',
        TRUE
    ),
    (
        'MODULO_EMPLEADOS_GESTIONAR',
        'Permite la gestión completa de la información de los empleados.',
        TRUE
    ),
    (
        'MODULO_PROVEEDORES_GESTIONAR',
        'Permite la gestión completa de la información de los proveedores.',
        TRUE
    ),
    (
        'MODULO_CATEGORIAS_PRODUCTOS_GESTIONAR',
        'Permite la gestión de las categorías de productos.',
        TRUE
    ),
    (
        'MODULO_CATEGORIAS_PRODUCTOS_VER',
        'Permite ver las categorías de productos (Cliente).',
        TRUE
    ),
    (
        'MODULO_CATEGORIAS_SERVICIOS_GESTIONAR',
        'Permite la gestión de las categorías de servicios.',
        TRUE
    ),
    (
        'MODULO_CATEGORIAS_SERVICIOS_VER',
        'Permite ver las categorías de servicios (Cliente).',
        TRUE
    ),
    (
        'MODULO_PRODUCTOS_GESTIONAR',
        'Permite la gestión completa de los productos del inventario.',
        TRUE
    ),
    (
        'MODULO_PRODUCTOS_VER',
        'Permite ver los productos (Cliente).',
        TRUE
    ),
    (
        'MODULO_COMPRAS_GESTIONAR',
        'Permite la gestión de las compras a proveedores.',
        TRUE
    ),
    (
    ...
    (
        'MODULO_VENTAS_CREAR_PROPIA',
        'Permite a un cliente crear/realizar una venta (compra).',
        TRUE
    ),
    (
        'MODULO_VENTAS_VER_PROPIAS',
        'Permite a un cliente ver sus propias ventas.',
        TRUE
    ),
    (
    ...
    (
        'MODULO_CITAS_CREAR_PROPIA',
        'Permite a un cliente agendar sus propias citas.',
        TRUE
    ),
    (
        'MODULO_CITAS_VER_PROPIAS',
        'Permite a un cliente ver sus propias citas.',
        TRUE
    ),
    (
        'MODULO_CITAS_CANCELAR_PROPIA',
        'Permite a un cliente cancelar sus propias citas (con antelación).',
        TRUE
    ),
    (
        'MODULO_SERVICIOS_GESTIONAR',
        'Permite la gestión completa de los servicios ofrecidos.',
        TRUE
    ),
    (
        'MODULO_SERVICIOS_VER',
        'Permite ver los servicios ofrecidos (Cliente).',
        TRUE
    ),
    (
    ...
    (
        ...
        'Permite ver productos desde la aplicación móvil.',
        TRUE
    ),
    (
        'MODULO_MOVIL_SERVICIOS_VER',
        'Permite ver servicios desde la aplicación móvil.',
        TRUE
    ),
    (
        'MODULO_MOVIL_CATEGORIAS_VER',
        'Permite ver categorías desde la aplicación móvil.',
        TRUE
    ),
    (
        'MODULO_MOVIL_EMPLEADOS_VER',
        'Permite ver empleados desde la aplicación móvil.',
        TRUE
    ),
    (
        'MODULO_MOVIL_NOVEDADES_VER',
        'Permite ver novedades desde la aplicación móvil.',
        TRUE
    ),
    (
        'MODULO_MOVIL_CLIENTES_VER_PROPIO',
        'Permite ver perfil propio desde la aplicación móvil.',
        TRUE
    ) ON CONFLICT (nombre) DO
UPDATE
...
SELECT (
    ...
    p.id_permiso
FROM permisos p
WHERE p.estado = TRUE ON CONFLICT (id_rol, id_permiso) DO NOTHING;
INSERT INTO permisos_x_rol (id_rol, id_permiso)
SELECT r.id_rol,
    p.id_permiso
FROM rol r,
...
INSERT INTO usuario (correo, contrasena, id_rol, estado)
VALUES (
        'mrgerito@gmail.com',
        '$2b$10$oJOJM36rGGzZftagNM1vWOxLaW96cPBRk.DhhvSvv8gneGTzFIJhO',
        (
            SELECT id_rol
            FROM rol
            WHERE nombre = 'Administrador'
...
VALUES (1, 'En proceso'),
    (2, 'Pendiente'),
    (3, 'Completado'),
    (4, 'Cancelado') ON CONFLICT (id_estado) DO
UPDATE
