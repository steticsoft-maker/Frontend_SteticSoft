...
const crearCita = async (datosCita) => {
  ...
  if (!novedad) {
    throw new BadRequestError(
      `La novedad con ID ${novedadId} no fue encontrada.`
    );
  }

  const empleadoValido = novedad.empleados.some(
    (emp) => emp.id_usuario === usuarioId
  );
  if (!empleadoValido) {
    throw new BadRequestError(
      `El empleado con ID ${usuarioId} no está asociado a la novedad seleccionada.`
    );
  }
  ...
  if (!fechaCita.isBetween(fechaInicioNovedad, fechaFinNovedad, "days", "[]")) {
    throw new BadRequestError(
      `La fecha de la cita está fuera del rango de fechas de la novedad.`
    );
  }
  ...
  if (!novedad.dias.includes(diaCita)) {
    throw new BadRequestError(
      `El día de la cita no está disponible en la novedad seleccionada.`
    );
  }
  ...
  if (horaCita < novedad.horaInicio || horaCita > novedad.horaFin) {
    throw new BadRequestError(
      `La hora de la cita está fuera del rango de horas de la novedad.`
    );
  }
  ...
  if (cliente.usuario.rol.nombre !== "Cliente") {
    throw new BadRequestError(
      `El usuario asociado al cliente no tiene el rol de "Cliente".`
    );
  }
  ...
  if (usuarioId) {
    ...
    if (empleado.rol.nombre !== "Empleado") {
      throw new BadRequestError(
        `El usuario (empleado) no tiene el rol de "Empleado".`
      );
    }
  }
  ...
  try {
    ...
    if (
      ...
      if (clienteNuevo.usuario.rol.nombre !== "Cliente") {
        await transaction.rollback();
        throw new BadRequestError(
          `El usuario asociado al nuevo cliente no tiene el rol de "Cliente".`
        );
      }
    ...
    if (datosActualizar.novedadId) {
      ...
      if (!novedad) {
        await transaction.rollback();
        throw new BadRequestError(
          `La novedad con ID ${datosActualizar.novedadId} no fue encontrada.`
        );
      }
      ...
      if (
        !fechaCita.isBetween(fechaInicioNovedad, fechaFinNovedad, "days", "[]")
      ) {
        await transaction.rollback();
        throw new BadRequestError(
          `La fecha de la cita está fuera del rango de fechas de la novedad.`
    ...
    if (
      datosActualizar.usuarioId &&
      datosActualizar.usuarioId !== cita.usuarioId
    ) {
      ...
      if (!empleadoNuevo) {
        await transaction.rollback();
        throw new BadRequestError(
          `Nuevo usuario (empleado) con ID ${datosActualizar.usuarioId} no encontrado o inactivo.`
        );
      }
      if (empleadoNuevo.rol.nombre !== "Empleado") {
        await transaction.rollback();
        throw new BadRequestError(
          `El nuevo usuario (empleado) no tiene el rol de "Empleado".`
        );
      }
    }
    if (
